...
namespace Bezier
{
	...
	template<typename T>
	inline T QuadraticInterpolate(T const& p1, T const& p2, T const& p3, float t) noexcept
	{
		return p1 * (1 - t) * (1 - t) +
			p2 * 2 * t * (1 - t) +
			p3 * t * t;
	...
	template<>
	inline dx::XMVECTOR QuadraticInterpolate(dx::XMVECTOR const& p1, dx::XMVECTOR const& p2, dx::XMVECTOR const& p3, float t) noexcept
	{
		using namespace DirectX;

		const XMVECTOR T0 = XMVectorReplicate((1 - t) * (1 - t));
		const XMVECTOR T1 = XMVectorReplicate(2 * t * (1 - t));
		const XMVECTOR T2 = XMVectorReplicate(t * t);
	...
	template<>
	inline dx::XMVECTOR CubicInterpolate(dx::XMVECTOR const& p1, dx::XMVECTOR const& p2, dx::XMVECTOR const& p3, dx::XMVECTOR const& p4, float t) noexcept
	{
		using namespace DirectX;
	...
	template<>
	inline dx::XMVECTOR CubicTangent(dx::XMVECTOR const& p1, dx::XMVECTOR const& p2, dx::XMVECTOR const& p3, dx::XMVECTOR const& p4, float t) noexcept
	{
		using namespace DirectX;
	...
	void CreatePatchVertices(_In_reads_(16) const dx::XMVECTOR patch[16], size_t tessellation, bool isMirrored, TOutputFunc outputVertex)
	{
		using namespace DirectX;
