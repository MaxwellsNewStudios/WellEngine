#include "stdafx.h"
#include "PhysicsForceBehaviour.h"
#include "Colliders/JoltColliderBehaviour.h"
...
bool PhysicsForceBehaviour::Start()
{
	if (_name.empty())
		_name = "PhysicsForceBehaviour"; // For categorization in ImGui.
...
bool PhysicsForceBehaviour::PhysicsUpdate(float deltaTime)
{
	if (JoltColliderBehaviour *collider; GetEntity()->GetBehaviourByType<JoltColliderBehaviour>(collider))
	{
...
bool PhysicsForceBehaviour::Serialize(json::Document::AllocatorType &docAlloc, json::Value &obj)
{
...
bool PhysicsForceBehaviour::Deserialize(const json::Value &obj, Scene *scene)
{
...
#ifdef USE_IMGUI
bool PhysicsForceBehaviour::Update(TimeUtils &time, const Input &input)
{
	if (_debugDraw)
	{
		dx::XMFLOAT3 wForce = GetWorldForce();
...
bool PhysicsForceBehaviour::RenderUI()
{
...
dx::XMFLOAT3 PhysicsForceBehaviour::GetWorldForce() const
{
...
dx::XMFLOAT3 PhysicsForceBehaviour::GetWorldPoint() const
{
	if (_localSpace) // Convert from local to world space
	{
		Transform *transform = GetTransform();
