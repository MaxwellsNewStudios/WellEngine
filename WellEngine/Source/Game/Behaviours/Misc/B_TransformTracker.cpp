...
bool B_TransformTracker::ParallelUpdate(const TimeUtils &time, const Input &input)
{
...
#ifdef USE_IMGUI
bool B_TransformTracker::RenderUI()
{
	// Drag & Drop target for tracking entities.
	{
		...
		{
			if (trackedEntity)
				GetScene()->GetDebugManager()->Select(trackedEntity);
		}
...
Entity *B_TransformTracker::GetTrackedEntity() const
{
	return _trackedEntity.Get();
}
const dx::XMFLOAT3 &B_TransformTracker::GetOffset()
{
	return _offset;
}

void B_TransformTracker::SetTrackedEntity(Entity *ent)
{
	_trackedEntity = ent;
}
void B_TransformTracker::GetOffset(dx::XMFLOAT3 offset)
{
	_offset = offset;
}

bool B_TransformTracker::Serialize(json::Document::AllocatorType &docAlloc, json::Value &obj)
{
...
bool B_TransformTracker::Deserialize(const json::Value &obj, Scene *scene)
{
