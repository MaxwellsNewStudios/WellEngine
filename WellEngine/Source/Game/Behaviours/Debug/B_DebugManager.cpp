...
#ifdef DEBUG_BUILD
bool B_DebugManager::Start()
{
	Scene *scene = GetScene();
	SceneHolder *sceneHolder = scene->GetSceneHolder();
	Content *content = scene->GetContent();

	if (scene->GetDebugManager())
	{
		if (scene->GetDebugManager() != this)
		{
			Warn("Scene already has a B_DebugManager, destroying this one.");
			Destroy();
			return true;
		}
	}
	...
	// Create main camera
	{
		...
		B_Camera *camera = new B_Camera(projInfo);
	...
	// Create orthographic camera
	{
		...
		B_Camera *camera = new B_Camera(projInfo, true);
	...
	GetScene()->SetMainCamera(_currCameraPtr.GetAs<B_Camera>());
...
bool B_DebugManager::Update(TimeUtils &time, const Input &input)
{
	Scene *scene = GetScene();
	SceneHolder *sceneHolder = scene->GetSceneHolder();
	...
	if (_mainCamera.IsValid())
	{
		...
		B_Camera *mainCam = _mainCamera.GetAs<B_Camera>();
	...
	if (!_secondaryCamera.IsValid())
	{
		...
		if (secondaryCam)
		{
			B_Camera *newSecondaryCam = nullptr;
			secondaryCam->GetBehaviourByType<B_Camera>(newSecondaryCam);
	...
	if (input.IsInFocus()) // Handle user input while window is in focus
	{
		...
		if (input.GetKey(KeyCode::M2) == KeyState::Pressed)
		{
			...
			if (newSelectEnt)
			{
				...
				{
					if (parent->HasBehaviourOfType<B_MeshBillboard>() || 
						parent->HasBehaviourOfType<B_MeshText>())
						newSelectEnt = parent;
				}
	...
	{ // Change camera
		...
		if (_currCamera < 0)
		{
			...
			{
				scene->SetMainCamera(_secondaryCamera.GetAs<B_Camera>());
			}
			else if (_mainCamera)
			{
				_currCamera = -2;
				scene->SetMainCamera(_mainCamera.GetAs<B_Camera>());
			}
		}
		else if (_currCamera < spotlightCount)
		{
			scene->SetMainCamera(spotlights->GetLightBehaviour(_currCamera)->GetShadowCamera());
		}
	}
...
bool B_DebugManager::Serialize(json::Document::AllocatorType &docAlloc, json::Value &obj)
{
	return true;
}
bool B_DebugManager::Deserialize(const json::Value &obj, Scene *scene)
{
...
bool B_DebugManager::HandleCameraMovement(TimeUtils &time, const Input &input)
{
	ZoneScopedXC(RandomUniqueColor());
	...
	if (input.IsCursorLocked())
	{
		...
		{
			B_Camera *cam = _currCameraPtr.GetAs<B_Camera>();
		...
		case MouseMovementMode::OrbitPan:
		{
			...
			else if (holdingM1 && dragMode != DragMode::None)
			{
				...
				else if (dragMode == DragMode::Pan)
				{
					...
					B_Camera *camBeh = _currCameraPtr.GetAs<B_Camera>();
...
bool B_DebugManager::IsSelected(Entity *ent, UINT *index, bool includeBillboard) const
{
	if (!ent)
		return false;
	...
	if (includeBillboard)
	{
		...
		{
			if (B_MeshBillboard *billboard; parent->GetBehaviourByType<B_MeshBillboard>(billboard))
			{
				if (IsSelected(parent, index))
					return true;
			}
			else if (B_MeshText *text; parent->GetBehaviourByType<B_MeshText>(text))
			{
				if (IsSelected(parent, index))
					return true;
			}
...
void B_DebugManager::Select(UINT id, bool additive)
{
	if (id == CONTENT_NULL)
		return;
...
void B_DebugManager::Select(Entity *ent, bool additive)
{
	if (!ent)
		return;
...
void B_DebugManager::Select(Entity **ents, UINT count, bool additive)
{
	if (!additive)
		ClearSelection();
...
void B_DebugManager::Deselect(UINT id)
{
	if (id == CONTENT_NULL)
		return;
...
void B_DebugManager::Deselect(Entity *ent)
{
	if (!ent)
		return;
...
void B_DebugManager::Deselect(Entity **ents, UINT count)
{
	if (!ents || count <= 0)
		return;
...
void B_DebugManager::DeselectIndex(UINT index)
{
...
void B_DebugManager::ClearSelection()
{
	_currSelection.clear();
}

const UINT B_DebugManager::GetSelectionSize() const
{
	return _currSelection.size();
}
Entity *B_DebugManager::GetPrimarySelection() const
{
	if (_currSelection.empty())
		return nullptr;
	return _currSelection.back().Get();
}
const std::vector<Ref<Entity>> &B_DebugManager::GetSelection()
{
	return _currSelection;
}
void B_DebugManager::GetParentSelection(std::vector<Ref<Entity>> &selectedParents)
{
...
void B_DebugManager::SetEditSpace(ReferenceSpace space)
{
	DebugData::Get().transformSpace = (int)space;
}
ReferenceSpace B_DebugManager::GetEditSpace() const
{
	return (ReferenceSpace)DebugData::Get().transformSpace;
}
void B_DebugManager::SetEditType(TransformationType type)
{
	DebugData::Get().transformType = (int)type;
}
TransformationType B_DebugManager::GetEditType() const
{
	return (TransformationType)DebugData::Get().transformType;
}
void B_DebugManager::SetEditOriginMode(TransformOriginMode mode)
{
...
void B_DebugManager::PositionWithCursor(Entity *ent)
{
...
void B_DebugManager::AddGizmoBillboard(B_MeshBillboard *gizmo)
{
	if (!gizmo)
		return;
...
void B_DebugManager::RemoveGizmoBillboard(B_MeshBillboard *gizmo)
{
	if (!gizmo)
		return;
...
void B_DebugManager::UpdateGizmoBillboards()
{
	DebugData &debugData = DebugData::Get();
...
bool B_DebugManager::RayCastFromCamera(RaycastOut &out)
{
	if (!_currCameraPtr)
		return false;
...
bool B_DebugManager::RayCastFromCamera(RaycastOut &out, XMFLOAT3A &pos, XMFLOAT3A &dir)
{
	if (!_currCameraPtr)
		return false;
...
bool B_DebugManager::RayCastFromMouse(RaycastOut &out, const Input &input)
{
	if (!_currCameraPtr)
		return false;
...
bool B_DebugManager::RayCastFromMouse(RaycastOut &out, XMFLOAT3A &pos, XMFLOAT3A &dir, const Input &input)
{
	if (!_currCameraPtr)
		return false;
