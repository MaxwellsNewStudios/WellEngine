...
B_CameraCube::B_CameraCube(const CameraPlanes &planes, bool hasCSBuffer)
{
...
bool B_CameraCube::Serialize(json::Document::AllocatorType &docAlloc, json::Value &obj)
{
	return true;
}
bool B_CameraCube::Deserialize(const json::Value &obj, Scene *scene)
{
	return true;
}


void B_CameraCube::GetAxes(UINT cameraIndex, XMFLOAT3A *right, XMFLOAT3A *up, XMFLOAT3A *forward)
{
	switch (cameraIndex)
	{
	case 0:
...
XMFLOAT4A B_CameraCube::GetRotation(UINT cameraIndex)
{
	XMFLOAT4A result;
...
XMFLOAT4X4A B_CameraCube::GetViewMatrix(UINT cameraIndex)
{
	XMFLOAT3A r, u, f;
...
XMFLOAT4X4A B_CameraCube::GetProjectionMatrix() const
{
...
XMFLOAT4X4A B_CameraCube::GetViewProjectionMatrix(UINT cameraIndex)
{
...
bool B_CameraCube::UpdateBuffers()
{
	if (!_isDirty)
		return true;
...
bool B_CameraCube::BindShadowCasterBuffers() const
{
	auto context = GetScene()->GetContext();
...
bool B_CameraCube::BindGeometryBuffers() const
{
	auto context = GetScene()->GetContext();
...
bool B_CameraCube::BindLightingBuffers() const
{
	auto context = GetScene()->GetContext();
...
bool B_CameraCube::BindTransparentBuffers() const
{
	auto context = GetScene()->GetContext();
...
bool B_CameraCube::BindViewBuffers() const
{
	auto context = GetScene()->GetContext();
...
bool B_CameraCube::StoreBounds(BoundingBox &bounds) // TODO: Change to use bounding sphere
{
...
void B_CameraCube::QueueGeometry(const RenderQueueEntry &entry)
{
	_geometryRenderQueue.emplace_back(entry);
}
void B_CameraCube::QueueTransparent(const RenderQueueEntry &entry)
{
	_transparentRenderQueue.emplace_back(entry);
}
void B_CameraCube::ResetRenderQueue()
{
...
void B_CameraCube::SortGeometryQueue()
{
	ZoneScopedXC(RandomUniqueColor());

	std::sort(_geometryRenderQueue.begin(), _geometryRenderQueue.begin());
}
void B_CameraCube::SortTransparentQueue()
{
	ZoneScopedXC(RandomUniqueColor());
...
const std::vector<RenderQueueEntry> &B_CameraCube::GetGeometryQueue() const
{
	return _geometryRenderQueue;
}
const std::vector<RenderQueueEntry> &B_CameraCube::GetTransparentQueue() const
{
	return _transparentRenderQueue;
}

UINT B_CameraCube::GetCullCount() const
{
	return _lastCullCount;
}
void B_CameraCube::SetCullCount(UINT cullCount) 
{
	_lastCullCount = cullCount;
}

void B_CameraCube::SetRendererInfo(const RendererInfo &rendererInfo)
{
	_rendererInfo = rendererInfo;
}
void B_CameraCube::SetFarZ(float farZ)
{
...
RendererInfo B_CameraCube::GetRendererInfo() const
{
	return _rendererInfo;
}
float B_CameraCube::GetNearZ() const
{
	return _cameraPlanes.nearZ;
}
float B_CameraCube::GetFarZ() const
{
	return _cameraPlanes.farZ;
}

ID3D11Buffer *B_CameraCube::GetCameraGSBuffer() const
{
	return _viewProjArrayBuffer.GetBuffer();
}
ID3D11Buffer *B_CameraCube::GetCameraCSBuffer() const
{
