...
B_Camera::B_Camera(const ProjectionInfo &projectionInfo, bool isOrtho, bool invertDepth)
{
...
void B_Camera::OnDirty()
{
	_isDirty = true;
	_recalculateBounds = true;
}

bool B_Camera::ParallelUpdate(const TimeUtils &time, const Input &input)
{
...
#ifdef USE_IMGUI
bool B_Camera::RenderUI()
{
	if (ImGui::Button("Control"))
		GetScene()->SetMainCamera(this);
...
bool B_Camera::Serialize(json::Document::AllocatorType &docAlloc, json::Value &obj)
{
...
bool B_Camera::Deserialize(const json::Value &obj, Scene *scene)
{
...
XMFLOAT4X4A B_Camera::GetViewMatrix()
{
	XMFLOAT3A r, u, f;
...
XMFLOAT4X4A B_Camera::GetProjectionMatrix(bool invert) const
{
...
XMFLOAT4X4A B_Camera::GetViewProjectionMatrix()
{
...
const ProjectionInfo &B_Camera::GetCurrProjectionInfo() const
{
	return _currProjInfo;
}

bool B_Camera::ScaleToContents(const std::vector<XMFLOAT4A> &nearBounds, const std::vector<XMFLOAT4A> &innerBounds)
{
	if (!_ortho)
		return false;
...
bool B_Camera::FitPlanesToPoints(const std::vector<XMFLOAT4A> &points)
{
	const float
...
bool B_Camera::BindDebugDrawBuffers() const
{
	auto context = GetScene()->GetContext();
...
bool B_Camera::BindShadowCasterBuffers() const
{
	return BindViewBuffers();
}
bool B_Camera::BindPSLightingBuffers() const
{
	auto context = GetScene()->GetContext();
...
bool B_Camera::BindCSLightingBuffers() const
{
	auto context = GetScene()->GetContext();
...
bool B_Camera::BindTransparentBuffers() const
{
	auto context = GetScene()->GetContext();
...
bool B_Camera::BindViewBuffers() const
{
	auto context = GetScene()->GetContext();
...
bool B_Camera::BindInverseBuffers() const
{
	auto context = GetScene()->GetContext();
...
ID3D11Buffer *const B_Camera::GetInverseBuffers()
{
	return _invCamBuffer->GetBuffer();
}

bool B_Camera::StoreBounds(BoundingFrustum &bounds, bool includeScale)
{
	if (_ortho)
		return false;
...
bool B_Camera::StoreBounds(BoundingOrientedBox &bounds, bool includeScale)
{
	if (!_ortho)
		return false;
...
void B_Camera::ViewRectToWorldTile(const dx::XMFLOAT4 &minMax, dx::BoundingFrustum &outFrustum) const
{
...
void B_Camera::ViewRectToWorldTile(const dx::XMFLOAT4 &minMax, dx::BoundingOrientedBox &outBox) const
{
	XMMATRIX projMatrix = Load(GetProjectionMatrix());
...
const CamBounds *B_Camera::GetLightGridBounds()
{
...
void B_Camera::QueueGeometry(const RenderQueueEntry &entry)
{
...
void B_Camera::QueueTransparent(const RenderQueueEntry &entry)
{
	_transparentRenderQueue.emplace_back(entry);
}
void B_Camera::ResetRenderQueue()
{
...
void B_Camera::SortQueue(std::vector<RenderQueueEntry> &queue, bool reverse, int sortModeOverride) const
{
	ZoneScopedXC(RandomUniqueColor());
...
void B_Camera::SortGeometryQueue()
{
	ZoneScopedXC(RandomUniqueColor());

	SortQueue(_geometryRenderQueue);
}
void B_Camera::SortTransparentQueue()
{
	ZoneScopedXC(RandomUniqueColor());

	SortQueue(_transparentRenderQueue, true, 2);
}
void B_Camera::SortOverlayQueue()
{
	ZoneScopedXC(RandomUniqueColor());
...
const std::vector<RenderQueueEntry> &B_Camera::GetGeometryQueue() const
{
	return _geometryRenderQueue;
}
const std::vector<RenderQueueEntry> &B_Camera::GetTransparentQueue() const
{
	return _transparentRenderQueue;
}
const std::vector<RenderQueueEntry> &B_Camera::GetOverlayQueue() const
{
	return _overlayRenderQueue;
}

UINT B_Camera::GetCullCount() const
{
	return _lastCullCount;
}

float B_Camera::GetFOV() const
{
	return _currProjInfo.fovAngleY;
}
bool B_Camera::GetOrtho() const
{
	return _ortho;
}
bool B_Camera::GetInverted() const
{
	return _invertedDepth;
}
CameraPlanes B_Camera::GetPlanes() const
{
	return _currProjInfo.planes;
}
float B_Camera::GetAspectRatio() const
{
	return _currProjInfo.aspectRatio;
}

void B_Camera::SetFOV(const float fov)
{
...
void B_Camera::SetOrtho(bool state)
{
	_ortho = state;
...
void B_Camera::SetInverted(bool state)
{
	_invertedDepth = state;
}
void B_Camera::SetPlanes(CameraPlanes planes)
{
...
void B_Camera::SetAspectRatio(float aspect)
{
...
void B_Camera::SetRendererInfo(const RendererInfo &rendererInfo)
{
	_rendererInfo = rendererInfo;
}
RendererInfo B_Camera::GetRendererInfo() const
{
	return _rendererInfo;
}

ID3D11Buffer *B_Camera::GetCameraVSBuffer() const
{
	return _viewProjBuffer.GetBuffer();
}
ID3D11Buffer *B_Camera::GetCameraGSBuffer() const
{
	if (_viewProjPosBuffer == nullptr)
		return nullptr;
...
ID3D11Buffer *B_Camera::GetCameraCSBuffer() const
{
	if (_posBuffer == nullptr)
		return nullptr;
...
void B_Camera::GetViewRay(const XMFLOAT2 &viewPos, XMFLOAT3 &origin, XMFLOAT3 &direction)
{
...
void B_Camera::BeginScreenFade(float duration)
{
	_screenFadeRate = 1.0f / duration;
}
void B_Camera::SetScreenFadeManual(float amount)
{
	_screenFadeAmount = amount;
}
float B_Camera::GetScreenFadeAmount() const
{
	return _screenFadeAmount;
}
float B_Camera::GetScreenFadeRate() const
{
