...
B_LightSpotSimple::~B_LightSpotSimple()
{
	if (!IsInitialized())
		return;
...
#ifdef USE_IMGUI
bool B_LightSpotSimple::RenderUI()
{
	if (ImGui::Button("Swap with Shadowcasting Variant"))
	{
		...
		B_LightSpot *shadowLight = new B_LightSpot(
...
void B_LightSpotSimple::OnEnable()
{
	LightSpotCollection *spotlights = GetScene()->GetSpotlights();
	if (spotlights)
	{
...
void B_LightSpotSimple::OnDisable()
{
	LightSpotCollection *spotlights = GetScene()->GetSpotlights();
	if (spotlights)
	{
...
void B_LightSpotSimple::OnDirty()
{
	_recalculateBounds = true;
}

bool B_LightSpotSimple::Serialize(json::Document::AllocatorType &docAlloc, json::Value &obj)
{
...
bool B_LightSpotSimple::Deserialize(const json::Value &obj, Scene *scene)
{
...
SimpleSpotLightBufferData B_LightSpotSimple::GetLightBufferData() const
{
...
void B_LightSpotSimple::SetLightBufferData(XMFLOAT3 color, float angle, float falloff, bool isOrtho, float fogStrength)
{
	_color = color;
	_angle = angle;
	_falloff = falloff;
...
bool B_LightSpotSimple::ContainsPoint(const XMFLOAT3A &point)
{
...
bool B_LightSpotSimple::IntersectsLightTile(const BoundingFrustum &tile)
{
	if (_recalculateBounds)
	{
		float reach = CalculateLightReach(_color, _falloff);
...
bool B_LightSpotSimple::IntersectsLightTile(const BoundingOrientedBox &tile)
{
	if (_recalculateBounds)
	{
		float reach = CalculateLightReach(_color, _falloff);
