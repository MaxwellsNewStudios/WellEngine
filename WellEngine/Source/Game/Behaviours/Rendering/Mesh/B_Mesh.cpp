...
bool B_Mesh::Update(TimeUtils &time, const Input &input)
{
	if (_updatePosBuffer)
	{
...
bool B_Mesh::RenderUI()
{
	...
	// LOD data
	{
		...
		B_Camera *camera = GetScene()->GetMainCamera();
		if (camera)
		{
...
bool B_Mesh::Serialize(json::Document::AllocatorType &docAlloc, json::Value &obj) 
{
	Content *content = GetScene()->GetContent();
...
bool B_Mesh::Deserialize(const json::Value &obj, Scene *scene)
{
...
bool B_Mesh::ValidateMaterial(const Material **material)
{
	if (!material)
	{
...
void B_Mesh::SetMeshID(UINT meshID, bool updateBounds)
{
	_meshID = meshID;
	_updateMatBuffer = true;
...
void B_Mesh::SetBlendStateID(UINT blendStateID)
{
	_blendStateID = blendStateID;
}
bool B_Mesh::SetMaterial(const Material *material, float discardSettings)
{
	if (IsInitialized())
	{
...
void B_Mesh::SetTransparent(bool state)
{
	_isTransparent = state;
}
void B_Mesh::SetOverlay(bool state)
{
	_isOverlay = state;
}
void B_Mesh::SetCastShadows(bool state)
{
	_castShadows = state;
}
void B_Mesh::SetShadowsOnly(bool state)
{
	_shadowsOnly = state;
}
void B_Mesh::SetAlphaCutoff(float value)
{
	_alphaCutoff = value;
	_updateMatBuffer = true;
}
void B_Mesh::SetColor(const dx::XMFLOAT4 &color)
{
	_baseColor = color;
}
void B_Mesh::SetBounds(BoundingOrientedBox &newBounds)
{
	_bounds = newBounds;
	_recalculateBounds = true;
}
void WellEngine::B_Mesh::SetCullMode(FaceCullingType cullMode)
{
	_cullMode = cullMode;
}

UINT B_Mesh::GetMeshID() const
{
	return _meshID;
}
UINT B_Mesh::GetBlendStateID() const
{
	return _blendStateID;
}
const Material *B_Mesh::GetMaterial() const
{
	return _material;
}
const dx::XMFLOAT4 &B_Mesh::GetColor() const
{
	return _baseColor;
}
float B_Mesh::GetAlphaCutoff() const
{
	return _alphaCutoff;
}
...
void B_Mesh::SetLastUsedLOD(UINT lodIndex, float normalizedDist)
{
	_lastUsedLODIndex = lodIndex;
	_lastUsedLODDist = normalizedDist;
}
UINT B_Mesh::GetLastUsedLODIndex() const
{
	return _lastUsedLODIndex;
}
float B_Mesh::GetLastUsedLODDist() const
{
	return _lastUsedLODDist;
}
UINT B_Mesh::GetLODCount() const
{
	return GetScene()->GetContent()->GetMesh(_meshID)->GetNrOfSubMeshes();
}

void B_Mesh::StoreBounds(BoundingOrientedBox &meshBounds)
{
