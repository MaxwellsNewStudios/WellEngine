...
B_MeshBillboard::B_MeshBillboard(const Material &material, 
...
B_MeshBillboard::~B_MeshBillboard()
{
#ifdef DEBUG_BUILD
	if (_gizmo)
	{
		...
		B_DebugManager *debugPlayer = scene->GetDebugManager();
...
bool B_MeshBillboard::Start()
{
	...
	if (_gizmo)
	{
		...
		B_DebugManager *debugPlayer = GetScene()->GetDebugManager();
		if (debugPlayer)
...
bool B_MeshBillboard::ParallelUpdate(const TimeUtils &time, const Input &input)
{
...
void B_MeshBillboard::OnEnable()
{
	_meshBehaviour.Get()->GetEntity()->Enable();
}
void B_MeshBillboard::OnDisable()
{
	_meshBehaviour.Get()->GetEntity()->Disable();
}

bool B_MeshBillboard::Serialize(json::Document::AllocatorType &docAlloc, json::Value &obj)
{
	/**code += 
...
bool B_MeshBillboard::Deserialize(const json::Value &obj, Scene *scene)
{
...
void B_MeshBillboard::SetSize(float size)
{
	_scale = size;
}
void B_MeshBillboard::SetRotation(float rotation)
{
	_rotation = rotation;
}

B_Mesh *B_MeshBillboard::GetMeshBehaviour() const
{
