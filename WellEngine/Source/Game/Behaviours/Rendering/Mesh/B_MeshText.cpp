...
bool B_MeshText::Start()
{
	...
	B_Mesh *meshBehaviour;
	entity->GetBehaviourByType<B_Mesh>(meshBehaviour);
...
#ifdef USE_IMGUI
bool B_MeshText::RenderUI()
{
	if (!_meshBehaviour.IsValid())
		return true;

	if (ImGui::Button("Select Mesh"))
		GetScene()->GetDebugManager()->Select(_meshBehaviour.Get()->GetEntity());
...
bool B_MeshText::Serialize(json::Document::AllocatorType &docAlloc, json::Value &obj)
{
	...
	if (B_Mesh *mesh; _meshBehaviour.TryGet(mesh))
	{
...
bool B_MeshText::Deserialize(const json::Value &obj, Scene *scene)
{
	if (obj.HasMember("Atlas"))
	{
		...
		if (id != CONTENT_NULL)
		{
			...
			if (newAtlas)
				newAtlas->AddListener((size_t)this, std::bind(&B_MeshText::RecreateMesh, this));
		}
...
bool B_MeshText::PostDeserialize()
{
	if (_meshBehaviour.IsValid())
	{
...
void B_MeshText::RecreateMesh()
{
	if (_meshID == CONTENT_NULL)
	{
		Warn("No mesh ID set for B_MeshText!");
		return;
	}
	...
	if (!fontAtlas)
	{
		Warn("Invalid font atlas ID set for B_MeshText!");
		return;
	}

	if (!_meshBehaviour.Get())
	{
		Warn("No valid B_Mesh found for B_MeshText!");
		return;
	}
	...
	{
		ErrMsg("Failed to set material for B_MeshText!");
		return;
	}
	...
	{
		delete meshData;
		Warn("Invalid mesh ID set for B_MeshText!");
		return;
	}
	...
	{
		delete meshData;
		Warn("Failed to initialize mesh for B_MeshText!");
		return;
	}
}

const std::string &B_MeshText::GetText() const
{
	return _text;
}
void B_MeshText::SetText(std::string_view text, bool skipRebuild)
{
	_text = text;
...
UINT B_MeshText::GetFontAtlasID() const
{
	return _fontAtlasID;
}
void B_MeshText::SetFontAtlasID(UINT id, bool skipRebuild)
{
...
UINT B_MeshText::GetMeshID() const
{
	return _meshID;
}

B_Mesh *B_MeshText::GetMeshBehaviour() const
{
